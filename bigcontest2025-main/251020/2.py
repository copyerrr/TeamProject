...
# 데이터 로드 및 전처리
# ============================================================================
df = pd.read_csv(r'C:\Users\142\bigcontest2025\bigcontest2025\251020\merged_data_korean_columns.csv')
...
# 결과 저장
output_file = 'quarterly_final_analysis_통계개선.csv'
qtr_df.to_csv(output_file, index=False, encoding='utf-8-sig')
print(f"\n결과 저장: {output_file}")

# 임계값 정보도 함께 저장
threshold_info = pd.DataFrame({
    '지표': ['매출_대폭변화', '매출_소폭변화', '신규고객_대폭변화', '신규고객_소폭변화', 
            'SQ_대폭변화', 'SQ_소폭변화', '계절성_매우강함', '계절성_강함', '계절성_보통'],
    '임계값': [SALES_LARGE_THRESHOLD, SALES_SMALL_THRESHOLD, NEW_LARGE_THRESHOLD, NEW_SMALL_THRESHOLD,
              SQ_LARGE_THRESHOLD, SQ_SMALL_THRESHOLD, SEASONALITY_VERY_STRONG, SEASONALITY_STRONG, SEASONALITY_MODERATE],
    '설명': ['매출 QoQ 대폭 변화 기준', '매출 QoQ 소폭 변화 기준', '신규고객 대폭 변화 기준', '신규고객 소폭 변화 기준',
            '직전분기 대폭 변화 기준', '직전분기 소폭 변화 기준', '계절성 매우 강함 기준', '계절성 강함 기준', '계절성 보통 기준']
})
threshold_info.to_csv('통계기반_임계값.csv', index=False, encoding='utf-8-sig')
print(f"임계값 정보 저장: 통계기반_임계값.csv")
